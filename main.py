...
#   compute_lateral_gap_ratio() แบบเดิม (คำนวณจาก whole-container average) พลาดไป
#
#   ROOT CAUSE: ค่า lateral gap เดิมเปรียบเทียบ "กรอบสี่เหลี่ยมรวมทั้งภาพ" (container
#   bounds ymax เทียบ cargo extent ymax) เป็นตัวเลขเดียว ซึ่งไม่ไวพอจะจับ "หลุม" ที่
#   เกิดขึ้นเฉพาะบางตำแหน่ง x เพราะตำแหน่งอื่นที่กล่องยื่นลึกกว่า (จากมุมมอง isometric
#   ที่พื้นตู้เป็นรูปคลื่น) จะ "กลบ" ค่าเฉลี่ยรวมจนดูเหมือนไม่มีปัญหา ยืนยันด้วยพิกเซล
#   จริง: ตรวจสอบ EC50-01/EC51-02 พบช่องว่างเฉพาะจุดลึกถึง 57-69px ณ ตำแหน่งกลางตั้ง
#   แต่ค่า lateral_gap_ratio ที่คำนวณแบบเดิมออกมาแค่ ~7-10% (ไม่ถึงเกณฑ์ 12%)
#
#   วิธีแก้: สแกน "ช่องว่างเฉพาะจุด" (local gap = โครงสร้างตู้ที่ต่ำสุด ณ ตำแหน่ง x
#   นั้น ลบด้วยขอบล่างคาร์โก้ ณ ตำแหน่ง x เดียวกัน) ตลอดความกว้างคาร์โก้ แล้วหาช่วงที่
#   ค่านี้ "สูงต่อเนื่องเป็นบริเวณกว้าง" (ไม่ใช่จุดเดียวโดดๆ) ซึ่งบ่งชี้หลุมจริง
#
#   ข้อควรระวังสำคัญ (พบจากการทดสอบไฟล์จริง) - มี 2 แหล่งสัญญาณรบกวนหลักที่ต้องกรอง:
#     1. ป้ายบอกระยะทาง/ตัวเลข (เช่น "0.9","1.8","2.7") วาดทับพื้นหลังสีขาว สร้าง
#        "จุดกระโดดสูงโดดๆ" (spike) ไม่ใช่หลุมต่อเนื่อง - กรองด้วย (ก) median smoothing
#        หน้าต่างกว้าง (11px) และ (ข) เช็ค "ความขรุขระ" (roughness) ของสัญญาณดิบ -
#        หลุมจริงมีรูปทรงสามเหลี่ยมลาดเอียงสม่ำเสมอ (roughness ต่ำ) ในขณะที่ noise
#        จะกระโดดสลับไปมา (roughness สูง)
#     2. โซนผนังหัวตู้/ประตูท้ายตู้ (ริมทั้ง 2 ข้างของคาร์โก้) มีแผงโครงสร้างตู้ทึบที่
#        ทำให้เกิด "gap ปลอม" ขนาดใหญ่คงที่เสมอ (~166-170px) ในทุกไฟล์แม้แต่ไฟล์ที่
#        ปลอดภัย 100% (ยืนยันจาก EC20-01) เพราะแผงนี้ไม่ใช่ตัวชี้วัดพื้นที่ว่างจากการ
#        โหลดสินค้า แต่เป็นโครงสร้างที่มีอยู่แล้วโดยธรรมชาติ - จึงตัดโซนริมทั้ง 2 ข้าง
#        ออกจากการสแกน (เหลือแค่ส่วนกลาง ~65% ของความกว้างคาร์โก้)
#
#   ผลการทดสอบ (6 ไฟล์): EC50-01/EC51-02 ตรวจพบ LATERAL_GAP_RISK ที่จุดที่ผู้ใช้วง
#   สีแดงไว้สำเร็จ (เดิม v24.2 พลาด) ส่วน EC20-01 (ปลอดภัย 100%) และ ED85-02 (มีแต่
#   noise) ไม่เกิด false positive ใหม่ ยืนยันด้วยตัวกรอง roughness/coverage
#
# v24.2 - แก้บั๊กตำแหน่งกรอบผิดของ LATERAL_GAP_RISK (ผู้ใช้ชี้ด้วยการวงสีแดงใน
#   ED86-03): เดิมค่า lateral gap คำนวณแค่ "ผลต่างความสูงรวม" โดยไม่ระบุว่าช่องว่าง
#   อยู่ฝั่งบนหรือล่าง ทำให้วาดกรอบผิดตำแหน่ง (ไปครอบคลุมตัวกล่องคาร์โก้แทนพื้นที่ว่าง
#   จริง) แก้ไขด้วย get_precise_lateral_gap_box() ที่เปรียบเทียบช่องว่างบน-ล่างแยกกัน
#   แล้ววาดกรอบเฉพาะฝั่งที่มีช่องว่างจริงมากกว่า
#
# v24.1 - แก้ปัญหาหลายจุดที่พบจากการตรวจสอบ /ooda /scout กับไฟล์จริง 6 ไฟล์:
#   1. TOP_ROW_MAJORITY_RATIO: เดิมใช้ "ANY column has cargo" (1 คอลัมน์เดียวก็พอ)
#      เพื่อหา top_y ของตั้ง ซึ่งเสี่ยงถูก "หน้าบนลาดเอียง" (isometric parallelogram
#      top face) ของกล่องเพื่อนบ้านที่สูงกว่า "ล้ำ" เข้ามาในช่วง x ของตั้งที่เตี้ยกว่า
#      พบจากไฟล์ EC50-01: กล่องเหลือง VCS1A ถูกวัดความสูงผิดจนใกล้เคียงกล่องเขียว
#      TSC1A ข้างเคียง แก้ไขด้วยเกณฑ์ majority vote (65% ของคอลัมน์ที่สุ่มตรวจ)
#   2. EDGE-BASED comparison สำหรับ STEP_DOWN_RISK: เดิมเปรียบเทียบค่าเฉลี่ยทั้ง
#      segment ซึ่งพังเมื่อมี perspective drift ภายใน segment เอง (พบจากไฟล์ ED85-02:
#      กรอบคลุมกล่องสีม่วงทั้งหมดผิดพลาด) แก้ไขด้วยการเปรียบเทียบเฉพาะค่าที่ขอบ
#   3. extract_unused_floor_mm(): ดึงค่า "Unused Floor" ที่พิมพ์บน PDF โดยตรง (ground
#      truth ไม่ต้องพึ่งพา pixel measurement) มาช่วยยืนยัน/ผ่อนเกณฑ์ gap ratio สำหรับ
#      เคส borderline (เช่น ED86-03: lateral_gap_ratio=11.6% ใกล้เกณฑ์ 12% มาก)
#   4. ปรับปรุง AI prompt เพิ่มคำเตือนเรื่องกล่องสีเข้ม/สีผิดปกติที่ AI อาจมองข้าม
#
# v24 - แก้ปัญหา ROOT CAUSE สำคัญที่สุด: ฟังก์ชันแบ่ง "ตั้ง" (stack) และ "กล่อง" (box)
#   ในเวอร์ชัน v22/v23.1 ใช้แค่ _find_dark_boundary_lines_1d (หา "dip" สีเข้มแคบๆ)
#   เพื่อหาตำแหน่งรอยต่อ ซึ่งใช้ได้ผลเฉพาะกรณีกล่อง 2 ใบสีเดียวกันมีเส้นขอบมืดบางๆ
#   คั่นเท่านั้น แต่ในสถานการณ์จริงส่วนใหญ่กล่องแต่ละใบ/แต่ละ SKU มักมีสีต่างกันชัดเจน
#   ซึ่งสร้าง "step change" ถาวรในค่าสี ไม่ใช่ dip แคบ ทำให้ตรวจจับพลาดเกือบทั้งหมด
#   นำไปสู่การรวมหลายตั้ง/กล่องเข้าด้วยกันผิดพลาด (under-segmentation รุนแรง)
#
#   วิธีแก้: เพิ่มสัญญาณตรวจจับ boundary อีก 2 ชนิด รวมกับของเดิม (union แทนที่):
#     1. COLOR-STEP boundary: เปรียบเทียบสี RGB เฉลี่ย (หลัง median smoothing) ระหว่าง
#        ตำแหน่งที่ติดกัน หากต่างกันเกิน threshold และคงอยู่ต่อเนื่อง ถือเป็นรอยต่อจริง
#     2. FLOOR/EDGE-JUMP boundary: เปรียบเทียบตำแหน่ง local floor (แนวนอน) หรือขอบ
#        ซ้าย/ขวา (แนวตั้ง) ระหว่างตำแหน่งที่ติดกัน หากกระโดดขึ้น/ลงมากพอ แสดงว่าเป็น
#        รอยต่อระหว่างกล่อง/ตั้งที่ความสูงต่างกัน แม้จะมีสีเดียวกันก็ตาม
#   ทั้ง dark-dip (เดิม) + color-step (ใหม่) + floor/edge-jump (ใหม่) ทำงานร่วมกัน
#   แบบ union ทดสอบยืนยันแล้วว่าช่วยแยกตั้ง/กล่องได้ถูกต้องมากขึ้นอย่างมีนัยสำคัญ
#
#   เพิ่มเติม: เพิ่ม VETO GATE สำหรับ REAR_LATERAL_IMBALANCE - เดิม (v21-v23.1) ใช้
#   FORCE เท่านั้น (ไม่ veto AI) เพราะกลัวว่า deterministic per-box segmentation ที่
#   (ตอนนั้น) ยังไม่แม่นยำพอจะ veto ผิดพลาดในกรณีที่มี occlusion จริง แต่หลังแก้ไข v24
#   (per-box segmentation แม่นยำขึ้นมาก) จึงเพิ่ม VETO แบบมีเงื่อนไข: ถ้า deterministic
#   segmentation มี coverage สูงพอและวัดได้ว่าความสูงระหว่างตั้งใกล้เคียงกัน ให้ veto
#   การอ้างของ AI (แก้ปัญหา ED85-02/EC20-01 ที่ AI สับสนจากสีเข้ม/สีผิดปกติ)
#
...
#
#   ข้อจำกัดที่ทราบและยอมรับ (ยืนยันจากการทดสอบ พบว่าเป็นข้อจำกัดพื้นฐาน ไม่ใช่บั๊ก):
...
#     FRONT view แต่ BACK view มองเห็นแค่ 1 ชั้นเนื่องจากมุมมอง/การบัง)
#   - SAME-COLOR ADJACENT STACKS: ตั้งหลายอันที่มีสีเดียวกันสนิทติดกัน ยังคงอาศัย
#     dark-dip เป็นหลักในการแยก ซึ่งบางครั้งอาจแยกได้ไม่ครบทุกเส้น (ไม่กระทบผลลัพธ์
#     ความเสี่ยงมากนัก เพราะตั้งที่ under-segment มักมีความสูงใกล้เคียงกันอยู่แล้ว)
#
...
# เดิม LATERAL_GAP_RISK (คำนวณจาก whole-container average) พลาดไป - ดู CHANGELOG
# หัวไฟล์สำหรับรายละเอียด root cause และวิธีแก้ทั้งหมด
...
def extract_unused_floor_mm(pdf_bytes: bytes):
    ...
    v24.1 NEW: เดิมระบบไม่เคยใช้ค่านี้เลย ทั้งที่เป็นสัญญาณที่แม่นยำที่สุดสำหรับ
    LATERAL_GAP_RISK/FRONT_EMPTY_RISK/REAR_EMPTY_RISK - ยืนยันจากไฟล์จริง: ED86-03
    และ EC25-01 (ที่มี gap จริง) ทั้งคู่มีค่า "Unused Floor" ที่ไม่ใช่ศูนย์ (17.7in,
    18.9in ตามลำดับ) ในขณะที่ไฟล์อื่นๆ ที่ไม่มีปัญหาเรื่อง gap รวม (EC20-01, EC50-01,
    EC51-02, ED85-02) ล้วนมีค่า "Unused Floor: 0 (in)" ตรงกันทุกไฟล์
...
def get_precise_lateral_gap_box(view_container, view_cargo):
    ...
    ตำแหน่งช่องว่างจริงอยู่ที่ "มุมล่างขวาของพื้นตู้" ซึ่งตรงกับค่าที่คำนวณได้:
    bottom_gap=40px มากกว่า top_gap=9px อย่างชัดเจน
    ...
    มากกว่าเท่านั้น (แถบตามความกว้างเต็มของคาร์โก้ x ความสูงเท่ากับช่องว่างจริง)
    """
    if not view_container or not view_cargo:
        return None
...
def _raw_local_gap_profile(px, x_range, y_search, step=LOCAL_GAP_SAMPLE_STEP_PX):
    ...
    (x, gap_หรือ_None, cargo_bottom, struct_bottom)"""
    results = []
...
#   จริง แล้วหาเส้นแบ่งภายในแต่ละกลุ่มด้วย 3 สัญญาณรวมกัน (v24): dark-dip (เดิม) +
#   color-step (ใหม่) + floor-jump (ใหม่)
...
def _find_color_step_boundaries(color_profile, min_distance=COLOR_STEP_MIN_DISTANCE,
    ...
    (v24 NEW - แก้ปัญหาหลักที่ dark-dip เดิมพลาดการเปลี่ยนสี)"""
    n = len(color_profile)
    boundaries = []
    last_boundary = -999
    i = 1
...
def _combined_boundaries(color_profile, position_profile, lum_profile, min_seg_width):
    ...
    แบบ union แล้ว dedupe/merge boundary ที่อยู่ใกล้กันเกินไป

    หมายเหตุสำคัญ (พบจากการทดสอบไฟล์จริงหลายไฟล์): เดิมพยายามปิด dark-dip ในทิศทาง
    แนวนอน (แบ่งตั้ง) เพราะพบ false trigger จากไฮไลท์/เงาสะท้อนบนพื้นผิวกล่องมันวาว
    แต่พบว่าการปิดทำให้เกิด under-segmentation รุนแรงกับกรณีตั้งหลายอันที่มีสีเดียวกัน
    สนิทติดกัน จึงตัดสินใจคง dark-dip ไว้ในทั้ง 2 ทิศทาง เนื่องจากแม้จะเกิด over-
    segmentation เป็นครั้งคราว แต่ผลกระทบต่อผลลัพธ์ความเสี่ยงสุดท้ายมีน้อยมาก ในขณะที่
    การไม่แบ่งเลย (under-segment) จากการปิด dark-dip ทำให้พลาดการแยกตั้ง/กล่องที่จำเป็น
    อย่างสิ้นเชิงในหลายไฟล์ ซึ่งร้ายแรงกว่ามาก
    """
...
def detect_boxes_in_stack(view_img, x0, x1, y_search_top, y_search_bottom, search_expand_px=25):
    ...
    # v24.1 FIX: เดิมใช้ "ANY column has cargo" (threshold เทียบเท่า 1 คอลัมน์เดียว
    # ก็พอ) เพื่อหา top_y ของตั้ง ซึ่งเสี่ยงถูก "หน้าบนลาดเอียง" ของกล่องเพื่อนบ้านที่
    # สูงกว่า "ล้ำ" เข้ามาในช่วง x ของตั้งที่เตี้ยกว่า แก้ไขด้วย majority-vote (65%)
    top_y = None
...
def build_stack_box_model_per_view(diagram_crop, layout, crop_w, crop_h, crop_y_start, container_bounds, cargo_extent):
    ...
    for view in ("FRONT", "BACK"):
        ...
        # ปกติจึงจะเชื่อถือพอจะ veto ได้)
    ...
    try:
        ...
        for view_label in ("FRONT", "BACK"):
            ...
            if should_flag_lateral and view_label not in _existing_risk_views("LATERAL_GAP"):
                ...
                # borderline ที่ pixel-based ratio วัดได้ใกล้เคียงเกณฑ์มาก
                ...
                # เหล่านี้ไม่มี "Unused Floor" ระบุด้วย
