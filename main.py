...
"จาก v25.92 พบว่า ไฟล์ที่แนบ มีจุดเสี่ยง เกินมาที่ front view 2 จุด"):
  ...
  เป๊ะทั้งคู่) + BACK 1 จุด
    (1) STEP_DOWN_RISK/hidden_behind FRONT idx=1 (x=837-915, drop=46.3%)
    (2) STEP_DOWN_RISK/pairwise      FRONT idx=2 (x=1032-1227, drop=28.8%)
  ROOT CAUSE (ยืนยันด้วยการนับ pixel จริง ไม่ใช่การเดา - อยู่ที่ "ด่านแรกสุด" ของทั้งระบบ):
  สี SKU ของกล่อง TTTCA (น้ำเงินเทา/slate) = (102,102,153) มี saturation = 0.333 ซึ่งต่ำกว่า
  เกณฑ์ sat>=0.55 ของ vivid_cargo_mask -> "ถูกตัดทิ้งทั้งหมด ไม่เหลือแม้แต่พิกเซลเดียว"
  ทั้งที่เป็นสีที่มีพื้นที่มากที่สุดในวิว (54,221px > ชมพู 42,307px > เขียว 20,968px) และเป็น
  กองที่สูงที่สุดของทั้งคัน -> เกิด "หุบเขาปลอม" ใน cargo_top_y ตรงโซนที่มองไม่เห็น:
    x=745-841 h~341px (ส้ม/ชมพู เห็น) | x=921-985 h~147px (slate มองไม่เห็น เหลือแต่เศษ
    กล่องแดง TB01E 30px/คอลัมน์) | x=993 top=-1 (ไม่มี cargo เลย) | x=1001-1073 h~343px (เขียว)
  -> ขอบซ้าย/ขอบขวาของโซนที่มองไม่เห็น กลายเป็น "รอยกระโดด" ปลอม 2 จุดพอดี
  เหตุผลที่แก้ที่ต้นเหตุ ไม่ใช่เพิ่ม guard ปลายทาง: guard ทั้งหมด (apex/edge-consensus/
  internal-jump) ตั้งอยู่บนสมมติฐานว่า cargo_top_y สะท้อนยอดกล่องจริง - เมื่อกล่องทั้งกอง
  หายไปตั้งแต่ต้นทาง การเพิ่ม guard จะเป็นการ "ปิดอาการ" และเสี่ยง false-negative กับไฟล์อื่น
  FIX: เพิ่ม Tier-2 acceptance ใน vivid_cargo_mask (เป็นการ "เพิ่ม" pixel เท่านั้น ไม่เคยตัด
  pixel ที่เดิมผ่านเกณฑ์ออกแม้แต่พิกเซลเดียว -> regression-safe by construction):
    Tier 1 (เดิม ไม่แตะเลย): mx>=50 และ sat>=0.55
    Tier 2 (ใหม่): mx>=50 และ sat>=0.25 และ "ไม่ใช่สีโครงสร้างตู้" (_structural_color_mask -
                   vectorized mirror ของ _p1b_is_structural_container_color) และ "เป็นสีเติม
                   แบนขนาดใหญ่" (จำนวน pixel ที่ RGB ตรงกันเป๊ะ >= 2,000)
  เงื่อนไข "สีเติมแบนขนาดใหญ่" คือหัวใจที่ทำให้ปลอดภัย (MaxLoad Pro เรนเดอร์กล่องเป็น flat fill
  สีเดียวเป๊ะทั้งหน้ากล่อง ไม่มี gradient) - แยกแยะได้ขาดจากข้อมูลจริง:
    กล่องจริง (102,102,153) 54,221px ผ่าน | anti-alias (117,145,180) 1px ไม่ผ่าน (ต่างกัน 5
    หมื่นเท่า) | เงาด้านข้าง (51,51,76) 403px ไม่ผ่าน (คงพฤติกรรมเดิม ปลอดภัยกว่า)
    สีโครงสร้างตู้ทุกสีถูกกรองออกก่อนแล้ว: (179,179,90) 14,835px / (255,255,133) 2,253px /
    (255,255,175) 1,151px -> struct=True ทั้งหมด
  ผลทดสอบจริง: EC05-01 hazardCount 3 -> 1 (กรอบ FRONT 2 จุดที่ผู้ใช้วงไว้หายไปทั้งคู่ถูกต้อง,
  BACK hidden_behind idx=2 ยังอยู่ - ตรวจสอบภาพแล้วว่าเป็น step-down จริงที่ท้ายตู้ (กล่อง
  TGT1G ม่วง/ฟ้าเตี้ยกว่าเพื่อนบ้านชัดเจน) ตรงกับที่ผู้ใช้ไม่ได้วงไว้)
  ความสูงที่วัดได้หลังแก้ สม่ำเสมอตรงกับภาพจริง (โหลดเต็มคัน หลังคาเป็นระนาบเดียว):
    FRONT 321/344/341px (เดิม 321/243/341) | BACK 337/339/321px (เดิม 195/243/321)
  ทดสอบความปลอดภัย 11 ข้อ ผ่านครบ: (1) ไฟล์สีสดปกติ mask เหมือนเดิมทุกพิกเซล (diff=0)
  (2) สีโครงสร้างตู้ 400k px ทั้ง 3 แบบ (rear-wall/rail/ผนังโทนฟ้า EB74) ไม่ถูกดูดเข้า cargo
  (3) กล่องสีหม่นถูกตรวจพบครบ (4) color-invariance 4 สีหม่นต่างกัน ตรวจเจอเท่ากันหมด
  (5) gradient/anti-alias ไม่ถูกดูดเข้ามา (6) สีหม่นพื้นที่เล็ก <2,000px คงพฤติกรรมเดิม
  (7) สีเทา/ดำพื้นที่ใหญ่ไม่ถูกนับ
  ทดสอบระดับ pipeline เต็ม (หลักฐานชี้ขาดว่าไม่มี regression): แปลงสี slate -> น้ำเงินสด
  (จำลอง "ไฟล์ที่ไม่มี cargo สีหม่นเลย" = ไฟล์ส่วนใหญ่ในระบบ) แล้วรันทั้ง 2 เวอร์ชัน ได้ผล
  ตรงกัน 100% (1 จุด, BACK idx=2 เหมือนกัน) - พิสูจน์ว่า v25.93 ไม่กระทบไฟล์กลุ่มเดิมเลย
  และพิสูจน์ซ้ำว่า 2 จุดที่ FRONT เป็น artifact จากการมองไม่เห็นสีล้วนๆ (เพราะพอสีกลายเป็น
  สีสด v25.92 เองก็ไม่ flag 2 จุดนั้นเช่นกัน)
  ทดสอบ color-invariance ระดับ pipeline: เปลี่ยน slate เป็นสีหม่นอื่น 3 สี ผลลัพธ์เหมือนเดิม
  ข้อจำกัดที่ต้องบอกตรงไปตรงมา: รอบนี้มีไฟล์จริงให้ทดสอบเพียงไฟล์เดียว (EC05-01) - ความ
  ปลอดภัยของไฟล์เดิมอาศัย (ก) คุณสมบัติเชิงโครงสร้างว่า Tier-2 "เพิ่มอย่างเดียวไม่เคยตัด"
  (ข) การทดสอบ recolor ข้างต้น (ค) unit-test 11 ข้อ - แนะนำให้รัน regression กับชุดไฟล์
  benchmark เดิม (EB63, EB66, EB08, CC45-all, KB03-02, PB01) ยืนยันอีกครั้งก่อน deploy
  หมายเหตุที่พบระหว่างตรวจสอบ (ยังไม่แก้ในรอบนี้ เพราะผู้ใช้ไม่ได้ระบุว่าผิด): Phase 1B ยังนับ
  คอลัมน์ได้เพียง 3 คอลัมน์ต่อวิว (FRONT ตรวจพบ 6 แล้วถูก reconcile กับ BACK ที่ได้ 3 จนเหลือ 3)
  ทั้งที่ในภาพมีตั้งกล่องมากกว่านั้น - ไม่กระทบผลลัพธ์ของไฟล์นี้ แต่เป็นจุดที่ควรตรวจสอบต่อ
================================================================================
...
# v25.93 NEW (สำคัญ - พบจริงจาก EC05-01-10-Sep-26 ที่ผู้ใช้แนบ 12-Sep-2026 พร้อมวงกลมสีส้ม
# ระบุว่า "มีจุดเสี่ยงเกินมาที่ front view 2 จุด"): ยืนยันด้วยข้อมูลสี pixel จริงระดับไฟล์ว่า
# vivid_cargo_mask (ด่านแรกสุดของทั้งระบบ - ทุกกลไกใช้ mask นี้เป็นฐาน) "มองไม่เห็นกล่องสินค้า
# สีหม่น (low-saturation) เลยแม้แต่พิกเซลเดียว"
#
# ROOT CAUSE (ยืนยันด้วยการนับ pixel จริงทั้ง FRONT region):
#   สี SKU ของกล่อง TTTCA (น้ำเงินเทา/slate) = (102,102,153) -> saturation = (153-102)/153
#   = 0.333 ซึ่ง "ต่ำกว่าเกณฑ์ sat>=0.55" ของ vivid_cargo_mask -> ถูกตัดทิ้งทั้งหมด
#   ทั้งที่เป็นสีที่มีพื้นที่มากที่สุดในวิว (54,221 px มากกว่าสีชมพู (255,0,255) ที่ 42,307 px
#   และมากกว่าสีเขียว (0,128,0) ที่ 20,968 px) และเป็น "กองที่สูงที่สุด" ของทั้งคันด้วย
# ผลกระทบที่วัดได้จริง (cargo_top_y ดิบของ FRONT):
#   x=745-841  h~341px  (กล่องส้ม/ชมพู - สีสด มองเห็น)
#   x=921-985  h~147px  <-- "หุบเขาปลอม" เพราะทั้งช่วงเป็นกล่อง slate ที่มองไม่เห็น
#                            เหลือแต่เศษกล่องแดงเข้ม TB01E (128,0,0) เพียง 30 px ต่อคอลัมน์
#   x=993      top=-1   <-- ไม่มี cargo เลย (ทั้งที่ในภาพจริงมีกล่อง slate เต็มพื้นที่)
#   x=1001-1073 h~343px (กล่องเขียว TAP1C - สีสด มองเห็น)
#   -> เกิด "รอยกระโดด" ปลอม 2 จุดที่ขอบของโซนที่มองไม่เห็น ทำให้ถูก flag ผิดพลาด 2 จุดพอดี
#      ตรงกับวงกลมสีส้มที่ผู้ใช้ทำเครื่องหมายไว้เป๊ะทั้ง 2 จุด:
#        (1) hidden_behind FRONT idx=1 (x=837-915, drop=46.3%) = ขอบซ้ายของโซนที่มองไม่เห็น
#        (2) pairwise      FRONT idx=2 (x=1032-1227, drop=28.8%) = ขอบขวาของโซนที่มองไม่เห็น
#
# ทำไมจึงแก้ที่ vivid_cargo_mask ไม่ใช่ที่ guard ของแต่ละกลไก: guard ปลายทางทั้งหมด (apex,
# edge-consensus, internal-jump ฯลฯ) ตั้งอยู่บนสมมติฐานว่า "cargo_top_y สะท้อนยอดกล่องจริง"
# เมื่อกล่องทั้งกองหายไปจาก mask ตั้งแต่ต้นทาง ข้อมูลที่ guard ใช้ตัดสินก็ผิดไปด้วยทั้งหมด
# การไปเพิ่ม guard ปลายทางจะเป็นการ "ปิดอาการ" ไม่ใช่แก้ต้นเหตุ และจะทำให้ไฟล์อื่นเสี่ยง
# false-negative แทน
#
# FIX (Tier-2 acceptance - เป็นการ "เพิ่ม" pixel เท่านั้น ไม่เคยตัด pixel ที่เดิมผ่านเกณฑ์ออก
# แม้แต่พิกเซลเดียว จึงไม่มีทางทำให้ไฟล์เดิมสูญเสีย cargo ที่เคยตรวจพบได้เลยโดยโครงสร้าง):
#   Tier 1 (เดิม ไม่แตะเลย): mx>=50 และ sat>=0.55
#   Tier 2 (ใหม่): mx>=50 และ sat>=0.25 และ "ไม่ใช่สีโครงสร้างตู้" และ "เป็นสีเติมแบนขนาดใหญ่"
#                  (จำนวน pixel ที่มีค่า RGB ตรงกันเป๊ะ >= _LOWSAT_CARGO_MIN_FLAT_FILL_PX)
#
# เงื่อนไข "สีเติมแบนขนาดใหญ่" คือหัวใจที่ทำให้ปลอดภัย - ยืนยันด้วยข้อมูลจริงว่าแยกแยะได้ขาด
# เพราะ MaxLoad Pro เรนเดอร์กล่องด้วย flat fill (สีเดียวเป๊ะทั้งหน้ากล่อง ไม่มี gradient):
#   สีกล่องจริง (102,102,153)          -> 54,221 px  (ผ่าน)
#   สี anti-alias ขอบกล่อง (117,145,180) ->      1 px  (ไม่ผ่าน - ต่างกัน 5 หมื่นเท่า)
#   สีเงาด้านข้างกล่อง (51,51,76)        ->    403 px  (ไม่ผ่าน - คงพฤติกรรมเดิมไว้ ปลอดภัยกว่า)
# และสีโครงสร้างตู้ทุกสีที่พบจริงในไฟล์ถูกกรองออกด้วย _structural_color_mask ก่อนแล้วทุกสี:
#   (179,179,90) 14,835 px / (255,255,133) 2,253 px / (255,255,175) 1,151 px -> struct=True ทั้งหมด
# (ใช้เกณฑ์เดียวกับ _p1b_is_structural_container_color ที่ผ่านการ verify มาแล้วทั้งระบบ
#  เพียงแต่เขียนใหม่เป็นแบบ vectorized เพื่อใช้กับทั้งภาพได้)
_LOWSAT_CARGO_MIN_SAT = 0.25           # ต่ำกว่า 0.333 ของสีกล่องจริงพอสมควร แต่ยังสูงกว่า
# สีเทา/ดำ/ขาวของตัวอักษรและเส้นตาราง (sat=0.00) มาก
_LOWSAT_CARGO_MIN_FLAT_FILL_PX = 2000  # ~1/5 ของหน้ากล่อง 1 ใบที่ matrix_scale=3 (~100x100px)
# ต่ำกว่าสีกล่องจริงที่วัดได้ (54,221) 27 เท่า แต่สูงกว่า noise/anti-alias (1-403) หลายเท่าตัว


def _structural_color_mask(region):
    """v25.93 NEW: เวอร์ชัน vectorized ของ _p1b_is_structural_container_color (ใช้เกณฑ์เดียวกัน
    ทุกประการทั้ง exact-list และ hue-pattern ทั้ง 2 ทิศทาง) - ใช้กรองสีโครงสร้างตู้ออกจาก
    Tier-2 ของ vivid_cargo_mask (ดู docstring เต็มด้านบน)"""
    r = region[:, :, 0].astype(np.int16)
    g = region[:, :, 1].astype(np.int16)
    b = region[:, :, 2].astype(np.int16)
...
def vivid_cargo_mask(region, min_blob_size=150):
    ...
    # --- v25.93 Tier 2: กล่องสินค้าสีหม่น (low-saturation flat fill) ---
    # ดู docstring เต็มด้านบน _LOWSAT_CARGO_MIN_SAT สำหรับหลักฐาน+เหตุผล (พบจริงจาก EC05-01)
        ...
        if len(keep_colors):
            ...
            for c in keep_colors:
                km |= ((region[:, :, 0] == c[0]) & (region[:, :, 1] == c[1])
                       & (region[:, :, 2] == c[2]))
            n_added = int((km & low_cand & (~raw_mask)).sum())
            if n_added:
                print(f"[LOWSAT_CARGO] เพิ่ม cargo สีหม่น {len(keep_colors)} สี "
                      f"({[tuple(int(v) for v in c) for c in keep_colors]}) รวม {n_added}px")
