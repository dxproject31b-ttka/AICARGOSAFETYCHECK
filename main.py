...
================================================================================
v25.94 (ผู้ใช้แนบ EC10-02-09-Sep-26.pdf เพื่อทดสอบเคส "สีเดียวกัน" 14-Sep-2026):

  ── ผลยืนยันสำคัญ: กลไกมุมทแยง (v25.93) ทำงานได้จริงในเคสสีเดียวกัน ──────────────
  EC10-02 BACK view เป็นหลักฐานที่รอมานานว่ากลไก front_row_stepdown ใช้งานได้จริง
  ในกรณีที่ "เพื่อนบ้านเป็นสีเดียวกัน" (ข้อจำกัดที่เคยบันทึกไว้ใน v25.93):
    กลุ่มเตี้ย x=[1007,1083] front_h=111 สี (0,255,0) เขียว
    เพื่อนบ้านขวา x=[1085,1161] front_h=275 สี (0,255,0) เขียว  <-- สีเดียวกันเป๊ะ
    -> drop=59.6% ตรวจพบถูกต้อง (กล่อง IATIC-D1 วางชั้นเดียว มีพื้นที่ว่างด้านบน)
  จุดชี้ขาดที่พิสูจน์ว่ากลไกเดิมมองไม่เห็นเลย 100%:
    silhouette ของ BACK เรียบสนิท (340,341,340,340,316) max-drop เพียง 7.3%
    silhouette_drop ของคู่ที่ตรวจพบ = -0.3% และ -18.4% (ติดลบ = เส้นขอบบนกลับสูงขึ้น)
    -> ต่ำกว่าเกณฑ์ 20% ของ pairwise/cross_view/tail_stepdown ทุกกลไก
  สรุปขอบเขตที่แคบลงจริง: guard color_dist ตรวจ "สีเหนือพื้นผิวแถวหน้า" เทียบกับ
  "สีหน้ากล่องแถวหน้า" เท่านั้น ไม่ได้เทียบกับเพื่อนบ้าน -> เพื่อนบ้านสีเดียวกันได้
  (เหลือข้อจำกัดเฉพาะกรณีที่ "แถวหลังที่อยู่เหนือขึ้นไป" เป็นสีเดียวกับแถวหน้า)

  ── บั๊กที่แก้ในรอบนี้: ตรวจซ้ำที่ตำแหน่งเดียวกัน ────────────────────────────────
  ROOT CAUSE: detect_front_row_stepdown วนเทียบ "ทุกคู่ที่ติดกัน" (i, i+1) ตามลำดับ x
  ทำให้กลุ่มเตี้ยกลุ่มเดียวที่มีเพื่อนบ้านสูงกว่า "ทั้งซ้ายและขวา" ถูก flag ซ้ำ 2 ครั้ง
  ที่ตำแหน่ง x เดียวกันเป๊ะ - ยืนยันจาก EC10-02 BACK กลุ่ม x=[1007,1083]:
      คู่(0,1): เทียบเพื่อนบ้านซ้าย front_h=207 -> drop=46.4%
      คู่(1,2): เทียบเพื่อนบ้านขวา front_h=275 -> drop=59.6%
  ได้ risk 2 รายการที่ mark_x_range=(1007,1083) เหมือนกันทุกประการ -> วาดกรอบทับกัน
  2 ชั้น และนับ hazardCount=3 ทั้งที่มีจุดเสี่ยงจริงเพียง 2 ตำแหน่ง (นับเกิน 1 จุด)
  FIX: dedup ตาม (mark_view, mark_x_range) เก็บรายการที่ drop_ratio สูงสุด (หลักฐาน
  แรงสุด = เทียบกับเพื่อนบ้านที่สูงกว่ามากที่สุด) - ใช้หลักการเดียวกับที่
  detect_step_down_crossview ใช้อยู่แล้ว (merge ตาม key เก็บ drop_ratio สูงสุด)
  ไม่กระทบไฟล์ที่จุดเสี่ยงอยู่คนละตำแหน่ง (x-range ต่างกัน -> ไม่ถูก dedup เลย)

  ── ground truth ที่ผู้ใช้ยืนยันในรอบนี้ (ใช้เป็น benchmark ต่อไป) ────────────────
    EC10-02 BACK  กล่องเขียว IATIC-D1 เตี้ย = เสี่ยงจริง (ตรวจพบถูกต้อง)
    EC10-02 FRONT กล่องน้ำเงิน HMTID-BN ปลายขวา = "ไม่เสี่ยง ซ้อนบนด้วยกล่องสีแดง
                  สูงเท่ากัน" -> ปัจจุบันไม่ flag ถูกต้องแล้ว (ไม่ต้องแก้)
                  หมายเหตุ: กลไกมุมทแยงปฏิเสธด้วย silhouette_drop=27.0% (>=15%) และ
                  hidden_ratio=0.500 (<0.60) ส่วนกลไกเดิมปฏิเสธด้วย Edge-Column
                  Global Consensus Guard (v25.68) - ทั้ง 2 ชั้นสอดคล้องกับความจริง
    EC05-01 BACK  กล่องน้ำเงินเตี้ย + ม่วงซ้อนบน รวมแล้วยังเตี้ยกว่าชมพูซ้อนสอง
                  ด้านตรงข้าม = เสี่ยงจริง (ตรวจพบถูกต้อง)
  ข้อมูล front_h variation ที่รวบรวมได้ (รถรุ่นเดียวกัน TTKA6WH ทั้งหมด):
      มีโพรงจริง : EC05-01 BACK 67.8% | EC52-02 FRONT 46.1% | EC05-01 FRONT 45.6%
                   EC52-01 BACK 43.9% | EC10-02 BACK 59.6% | EC10-02 FRONT 48.5%
      บรรทุกเต็ม : EC52-01 FRONT 2.4% | EC52-02 BACK 1.5%
  -> แยกกลุ่มได้สะอาดมาก (ช่องว่าง 41.5 จุด) ยืนยันว่า front_h เป็นสัญญาณที่ถูกต้อง
  ข้อควรทราบ (บอกตรงไปตรงมา): front_h variation "ทั้ง view" ใช้เป็นเกณฑ์เดี่ยวไม่ได้
  เพราะไฟล์ที่มี SKU ขนาดต่างกันตามธรรมชาติได้ค่าสูงเช่นกัน (PB01 82.7%, CC45 79.2%,
  KB03 44.4%, CC19 34.4% - ทั้งหมดผู้ใช้ยืนยันว่าไม่มีจุดเสี่ยงใหม่) จึงต้องคงการ
  เทียบ "เป็นคู่ระหว่างกลุ่มที่ติดกัน + guard 4 ชั้น" ตามที่ v25.93 ออกแบบไว้เดิม

  ผลทดสอบ regression ครบ 8 ไฟล์ (v25.93 -> v25.94):
    EC10-02   3 -> 2  (ตัดกรอบซ้ำออก 1 จุด ตามที่ตั้งใจ)
    CC19-all  2 -> 2  | EC05-01  3 -> 3  | EC52-01  2 -> 2  | EC52-02  1 -> 1
    CC45      2 -> 2  | KB03     1 -> 1  | PB01     2 -> 2
  -> เปลี่ยนเฉพาะไฟล์เป้าหมายไฟล์เดียว ไม่มี regression ในไฟล์อื่นเลยแม้แต่จุดเดียว
================================================================================
...
def detect_front_row_stepdown(view_result, view_label, cells, down_factor):
    ...
    # v25.94 FIX (สำคัญ - พบจริงจาก EC10-02 ที่ผู้ใช้แนบ 14-Sep-2026 เพื่อทดสอบเคส "สีเดียวกัน"):
    # เดิมลูปเทียบ "ทุกคู่ที่ติดกัน" (i, i+1) ตามลำดับ x - ทำให้กลุ่มเตี้ยกลุ่มเดียวที่มีเพื่อนบ้าน
    # สูงกว่า "ทั้งซ้ายและขวา" ถูก flag ซ้ำ 2 ครั้งที่ตำแหน่ง x เดียวกันเป๊ะ
    # ยืนยันด้วยข้อมูลจริง EC10-02 BACK: กลุ่ม x=[1007,1083] (front_h=111) ถูกเทียบกับ
    #   คู่(0,1): เพื่อนบ้านซ้าย x=[931,1006] front_h=207 -> drop=46.3%
    #   คู่(1,2): เพื่อนบ้านขวา x=[1085,1161] front_h=275 -> drop=59.6%
    # ได้ risk 2 รายการที่ mark_x_range=(1007,1083) เหมือนกันทุกประการ -> วาดกรอบทับกัน 2 ชั้น
    # และนับ hazardCount=3 ทั้งที่มีจุดเสี่ยงจริงเพียง 2 ตำแหน่ง (นับเกินจริง 1 จุด)
    # FIX: dedup ตาม (mark_view, mark_x_range) เก็บรายการที่ drop_ratio สูงสุดไว้ (หลักฐานแรงสุด
    # = เทียบกับเพื่อนบ้านที่สูงกว่ามากที่สุด) - ใช้หลักการเดียวกับที่ detect_step_down_crossview
    # ใช้อยู่แล้ว (merge ตาม (mark_view, mark_stack_idx) เก็บ drop_ratio สูงสุด) เพื่อความสอดคล้อง
    # ไม่กระทบไฟล์ที่มีจุดเสี่ยงกระจายคนละตำแหน่ง (x-range ต่างกัน -> ไม่ถูก dedup เลย)
    if len(risks) > 1:
        merged = {}
        for r in risks:
            key = (r["mark_view"], tuple(r["mark_x_range"]))
            if key not in merged or r["drop_ratio"] > merged[key]["drop_ratio"]:
                if key in merged:
                    print(f"[FRONT_ROW_STEPDOWN] dedup: x={list(r['mark_x_range'])} "
                          f"view={r['mark_view']} มี 2 รายการ (เทียบเพื่อนบ้านคนละฝั่ง) "
                          f"-> เก็บ drop={r['drop_ratio']:.1%} (แทน "
                          f"{merged[key]['drop_ratio']:.1%})")
                merged[key] = r
            else:
                print(f"[FRONT_ROW_STEPDOWN] dedup: x={list(r['mark_x_range'])} "
                      f"view={r['mark_view']} ซ้ำ -> ตัด drop={r['drop_ratio']:.1%} ทิ้ง "
                      f"(เก็บ {merged[key]['drop_ratio']:.1%} ที่แรงกว่า)")
        risks = sorted(merged.values(), key=lambda r: r["mark_x_range"][0])
    return risks
...
def process_request(request):
    ...
    try:
        ...
        return ({
            ...
            "benchmarkMode": "v25_94_front_row_stepdown_dedup",
