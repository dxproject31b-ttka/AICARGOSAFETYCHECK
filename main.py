...
def generate_action_report(case_type, description="", sku_list=""):
    ...
    actions = {
        ...
        # v24.34 NEW - see LocalFloorGapWiringFix changelog entry near the top of this file.
        "LOCAL_FLOOR_GAP_RISK": (
            f"แจ้งเตือน: พบพื้นที่ว่างเฉพาะจุดบนพื้นตู้ใต้/หลังกองสินค้า (หลุมพื้นเฉพาะจุด){sku_line}\n"
            f"วิธีแก้ไข:\n"
            f"  • ตรวจสอบพื้นตู้บริเวณจุดที่ระบุ ว่ามีพื้นที่ว่างใต้/หลังกองสินค้าจริงหรือไม่\n"
            f"  • นำไม้อัดกั้นหรือวัสดุรองอุดช่องว่าง ป้องกันกองสินค้าทรุด/เคลื่อนลงหลุม\n"
            f"  • รัดด้วยสายเบลท์หรือเชือกยึดกองสินค้าบริเวณนี้ให้มั่นคงเพิ่มเติม"
        ),
    }
...
def _merge_narrow_similar_stacks_in_cluster(ranges, color_profile, cluster_x0, px=None,
    ...
    def _avg_color_from_profile(x0, x1):
        ...
        if idx1 <= idx0:
            return None
        seg = color_profile[idx0:idx1]
        rs = [c[0] for c in seg]; gs = [c[1] for c in seg]; bs = [c[2] for c in seg]
        return (sum(rs) / len(rs), sum(gs) / len(gs), sum(bs) / len(bs))

    merged = [list(ranges[0])]
    for i in range(1, len(ranges)):
        cur_x0, cur_x1 = ranges[i]
    ...
    if len(relevant) < 2:
        if globals().get("V2405_REAR_LATERAL_TRACE", True):
            print(f"v24.34 REAR_LATERAL (FORCE) {view_label}: only {len(relevant)} stack(s) "
                  f"overlap the rear zone x=[{rear_x0}-{rear_x1}] - cannot form any comparison "
                  f"pair (NOT evidence of safety; deferring entirely to the independent AI "
                  f"zone-call result for this view)")
        return []
    ...
    if len(suspect_indices) >= len(relevant) - 1 and len(relevant) >= 2:
        if globals().get("V2405_REAR_LATERAL_TRACE", True):
            print(f"v24.34 REAR_LATERAL (FORCE) {view_label}: {len(relevant)} stack(s) in rear "
                  ...
                  f"no reliable pair can be formed (NOT evidence of safety; deferring entirely "
                  f"to the independent AI zone-call result for this view)")
        return []
    regions = []
...
def _is_quota_error_message(msg):
    ...
    return (
        ...
        or ("504" in msg) or ("DEADLINE_EXCEEDED" in msg) or ("DEADLINE EXCEEDED" in msg)
...
def _is_permanently_bad_key_error(msg):
    ...
    return (
        ...
        or ("PERMISSION_DENIED" in msg) or ("UNAUTHENTICATED" in msg) or ("403" in msg)
    )
...
def _get_available_gemini_models(current_key=None):
    ...
    return [m for m in ordered if now >= MODEL_DISABLED_UNTIL.get((current_key, m), 0)]


def _mark_gemini_model_disabled(model_name, reason="", current_key=None):
    MODEL_DISABLED_UNTIL[(current_key, model_name)] = time.time() + MODEL_COOLDOWN_SECONDS
    print(f"AI MODEL QUOTA COOLDOWN: {model_name} (this key only) disabled for {MODEL_COOLDOWN_SECONDS}s reason={str(reason)[:120]}")
...
def _call_gemini_json(prompt, image, api_keys, known_bad_keys=None):
    global GLOBAL_KEY_INDEX, LAST_WORKING_MODEL
    if known_bad_keys is None:
        known_bad_keys = set()
    last_err = ""
    ...
    for i in range(total_keys):
        ...
        if current_key in known_bad_keys:
            print(f"API Key index {current_index} SKIPPED - already confirmed bad earlier in this request")
            continue
        try:
            _reset_genai_client()
            genai.configure(api_key=current_key)
            models = _get_available_gemini_models(current_key=current_key)
            if not models:
                last_err = "All Gemini Flash models are temporarily disabled by quota cache for this key"
                print(f"AI DISABLED: all Gemini Flash models are in quota cooldown for key_index={current_index}")
                continue
            for model_name in models:
                try:
                    print(f"AI MODEL ACTIVE: {model_name} key_index={current_index}")
                    model = genai.GenerativeModel(model_name=model_name)
                    response = model.generate_content([prompt, image], request_options=_gemini_request_options())
                    ...
                    if _is_quota_error_message(last_err):
                        print(f"AI FALLBACK: {model_name} -> next model (same key) reason={last_err[:100]}")
                        _mark_gemini_model_disabled(model_name, last_err, current_key=current_key)
                        continue
                    raise
        except Exception as e:
            last_err = str(e)
...
def analyze_rear_zone_with_ai(rear_crop, api_keys, view_label="UNKNOWN", known_bad_keys=None):
    """
...
"""
    return _call_gemini_json(prompt, rear_crop, api_keys, known_bad_keys=known_bad_keys)


def analyze_front_zone_with_ai(front_crop, api_keys, view_label="UNKNOWN", known_bad_keys=None):
    prompt = f"""
...
"""
    return _call_gemini_json(prompt, front_crop, api_keys, known_bad_keys=known_bad_keys)


def analyze_diagram_image_with_ai(diagram_image, layout="TOP_BOTTOM", known_bad_keys=None):
...
"""
    ...
    for i in range(len(api_keys)):
        ...
        current_key = api_keys[current_index]
        if current_key in known_bad_keys:
            print(f"API Key index {current_index} SKIPPED (diagram) - already confirmed bad earlier in this request")
            continue
        try:
            _reset_genai_client()
            genai.configure(api_key=current_key)
            models = _get_available_gemini_models(current_key=current_key)
            if not models:
                last_error_msg = "All Gemini Flash models are temporarily disabled by quota cache for this key"
                print(f"AI DISABLED: all Gemini Flash models are in quota cooldown for key_index={current_index}")
                continue
            for model_name in models:
                try:
                    ...
                    model = genai.GenerativeModel(model_name=model_name)
                    response = model.generate_content([prompt, diagram_image], request_options=_gemini_request_options())
                    clean_text = clean_json_response(response.text if response.text else "[]")
                    ...
                    if _is_quota_error_message(last_error_msg):
                        print(f"AI FALLBACK: {model_name} -> next model (same key) reason={last_error_msg[:100]}")
                        _mark_gemini_model_disabled(model_name, last_error_msg, current_key=current_key)
                        continue
                    raise
        except Exception as e:
...
def process_request(request):
    ...
    try:
        ...
        raw_ai_risks = analyze_diagram_image_with_ai(diagram_crop, layout=layout, known_bad_keys=known_bad_gemini_keys)
        ...
        rear_result_front = analyze_rear_zone_with_ai(rear_crop_front, api_keys_pool, "FRONT", known_bad_keys=known_bad_gemini_keys)
        rear_result_back = analyze_rear_zone_with_ai(rear_crop_back, api_keys_pool, "BACK", known_bad_keys=known_bad_gemini_keys)
        front_result_from_front_view = analyze_front_zone_with_ai(front_crop_front, api_keys_pool, "FRONT", known_bad_keys=known_bad_gemini_keys)
        ...
        if globals().get("V2434_LOCAL_FLOOR_GAP_RISK_ENABLED", True):
            ...
            for view_label in ("FRONT", "BACK"):
                for region in local_depth_gap_regions.get(view_label, []):
                    region_box = (region["x_min"], region["y_min"], region["x_max"], region["y_max"])
                    already_covered = False
                    for r in all_risks:
                        if str(r.get("view", "")).upper().strip() != view_label:
                        ...
                        if not r_abs:
                            continue
                        if _box_iou_absolute(region_box, r_abs) >= overlap_max:
                            already_covered = True
                            if globals().get("V2434_TRACE", True):
                                print(f"v24.34 LOCAL_FLOOR_GAP_RISK ({view_label}) skipped - "
                                      f"x=[{region['x_min']:.0f}-{region['x_max']:.0f}] overlaps "
                                      f"an already-collected {r.get('risk_type','')} marker "
                                      f"(same physical spot, avoiding duplicate marker)")
                            break
                    if already_covered:
                        continue
                    ymin_norm = ((region["y_min"] - crop_y_start) / crop_h) * 1000
                    ...
                    all_risks.append({
                        ...
                        "box_2d": [ymin_norm, xmin_norm, ymax_norm, xmax_norm],
                        "reasoning": "FORCED_DETERMINISTIC_LOCAL_DEPTH_GAP",
                        "description": f"พบพื้นที่ว่างเฉพาะจุดบนพื้นตู้ กว้างประมาณ {region['width_px']:.0f}px ลึกสูงสุดประมาณ {region['max_gap_px']:.0f}px (ตรวจจับจาก local depth-gap scan)",
                    })
        ...
        for risk in all_risks:
            ...
            if not drawn and not is_zone_based and box and isinstance(box, list) and len(box) == 4:
                try:
                    ...
                    elif is_forced_local_floor_gap_marker:
                        min_w_px = globals().get("V2413_STEPDOWN_MIN_ABS_WIDTH_PX", 6)
                        min_h_px = globals().get("V2413_STEPDOWN_MIN_ABS_HEIGHT_PX", 6)
                        box_too_small = (abs_xmax - abs_xmin) < min_w_px or (abs_ymax - abs_ymin) < min_h_px
                        box_too_large = False
                        if globals().get("V2434_TRACE", True):
                            print(f"v24.34 LOCAL_FLOOR_GAP_RISK forced marker - ratio size gate SKIPPED "
                                  f"(w_px={abs_xmax - abs_xmin}, h_px={abs_ymax - abs_ymin})")
                    else:
